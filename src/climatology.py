...
def get_variable_climatology(tile: Tuple[float, ...], var: str, start_year: int, end_year: int):
    """
...
def get_complementarity_index(tile: Tuple[float, ...], start_year: int, end_year: int):
    ...
    """
    ds_wind = load_era5_variable(ERA5_ZARR_URL, "ws100", tile, start_year, end_year, daily_sum=False)
    ds_solar = load_era5_variable(ERA5_ZARR_URL, "ssrd", tile, start_year, end_year, daily_sum=True)
    
    if ds_wind is None or ds_solar is None: return None
